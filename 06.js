...
const letterDB = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";

const processedInput = input.split("\n");
let newArray = [];
let priorities = 0;

// creates a new array by grouping every next 3 items together.
for (let i = 1; i < processedInput.length / 3 + 1; i++) {
  newArray.push(processedInput.slice(-3 + i * 3, 0 + i * 3));
}
...
newArray.forEach((arr) => {
  ...
  for (let i = 0; i < arr[0].length; i++) {
    let check = arr[1].includes(arr[0][i]) && arr[2].includes(arr[0][i]) ? 1 : 0;

    console.log(check);
    if (check === 1 && mainLetter !== arr[0][i]) {
      priorities = priorities + letterDB.indexOf(arr[0][i]) + 1;
      mainLetter = arr[0][i];
    }
  }
});
